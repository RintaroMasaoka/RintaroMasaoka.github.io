...
Fuzzy sphere regularization は、3d CFT を $S^2\times\mathbb{R}$ 上の有限サイズ量子多体系として調べる方法である。Finite-size Hamiltonian の spectrum、matrix element、correlator から、CFT の conformal data を抽出する。

Spherical Landau level は、有限個の orbital だけを残しながら $SO(3)$ を保つ cutoff を与える。この一粒子空間の上に many-body Hamiltonian を置くと、3d Ising CFT の spectrum、OPE coefficients、four-point correlator、defect CFT data を同じ枠組みで扱える。
...
radial quantization では 3d CFT の空間は、空間は $S^2$、時間は $\mathbb{R}$ である。
...
spherical LLL は有限個の orbital を与えるが、それだけでは物理模型は決まらない。fuzzy sphere regularization で CFT data を抽出するには、その orbital 上に many-body Hamiltonian を置き、対称性を保ったまま相互作用を選び、パラメータを tune して目的の IR critical point に近づける必要がある。
...
critical point 近くで exact diagonalization した低エネルギー状態は、$SO(3)$ spin $L$、Ising $\mathbb{Z}_2$、spacetime parity $P$ で分類される。CFT では、これらを operator の $SO(3)$ spin $\ell$、$\mathbb{Z}_2$ parity、spacetime parity に対応させる。
...
主表 では、$N=16$ の fuzzy-sphere spectrum から次の primary operators が同定されている。
...
同じ量子数の中で最も小さい $\Delta_\beta$ を持つ operator が、large-$R$ limit の leading contribution になる。例えば odd scalar probe なら、通常は $\sigma$ が最初に残り、$\sigma'$ などは $R^{-(\Delta_{\sigma'}-\Delta_\sigma)}$ だけ余分に抑えられる。したがって fuzzy sphere の有限サイズデータでは、この leading term に近づき方を調べて、$R\to\infty$ へ外挿する。
...
7. 有限サイズデータ を $R\to\infty$ へ外挿する。実用上は $R^{-2}\sim1/N$ を fit 変数 にする。
8. 別 probe、別 fit 範囲、既知 bootstrap value と比較して 誤差を見積もる。
