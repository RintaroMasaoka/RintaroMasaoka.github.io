...
主な対象は Hu, He, Zhu の magnetic line defect paper である。
...
Magnetic line defect は、全ての $S^2$ time slices の north/south pole に置かれた時間非依存 Hamiltonian term として入る。そこから $h_d$ による flow、residual symmetry による spectrum label、defect state-operator correspondence、displacement operator の同定へ進む。結果表だけを見ると、bulk spectrum の読み方と defect spectrum の読み方が混ざりやすい。
